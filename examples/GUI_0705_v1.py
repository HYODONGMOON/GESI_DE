...
def save_to_excel():
    ...
    # 공통 #
    target_year = float(target_year_entry.get())
    ...
    ws2['B1'] = float(target_year)


    wb.save('C:\\Users\\GESI Moon\\Desktop\\python workplace\\GESI_DE - GUI\\examples\\GESI_DE.xlsx')
    wb.close()

    gesi.run_analysis(city_area, industry_area, rural_area, port_area)

   
def run_script():
    # 엑셀 파일 경로
    excel_file_path = 'C:\\Users\\GESI Moon\\Desktop\\python workplace\\GESI_DE - GUI\\examples\\GESI_DE.xlsx'
    ...
    # 엑셀 창 활성화 후 저장 실행
    pyautogui.hotkey('alt', 'f')     # "Alt" 키를 누른 상태에서 "F" 키를 눌러 파일 메뉴에 접근합니다.
    pyautogui.press('s')    # "S" 키를 눌러 "저장" 메뉴에 접근합니다.
    time.sleep(1)    # 저장 대화상자가 나타날 때까지 잠시 대기합니다.
    pyautogui.press('enter')     # "Enter" 키를 눌러 저장을 수행합니다.
...
# city_area_entry.insert(0, "100")  # 100을 기본값으로 설정
# city_area_entry.config(fg="light grey")  # 글자색을 옅은 회색으로 설정
...
def apply_default_values():
    frames = [principles_frame, scenario_frame_city, scenario_frame_industry, scenario_frame_rural, scenario_frame_port]  # 기본값을 설정할 프레임들
    for frame in frames:
        for widget in frame.winfo_children():
            if isinstance(widget, tk.Entry):
                widget.insert(0, "0")
                widget.config(show=" ")  # 기본값 0을 숨기는 설정
                widget.bind("<FocusIn>", lambda event, widget=widget: clear_default_value(event, widget))  # 포커스가 들어올 때 기본값 삭제
                widget.bind("<FocusOut>", lambda event, widget=widget: restore_default_value(event, widget))  # 포커스가 나갈 때 기본값 복원

def clear_default_value(event, widget):
    if widget.get() == "0":
        widget.delete(0, tk.END)
        widget.config(show="")  # 숨김 설정 제거

def restore_default_value(event, widget):
    if widget.get() == "":
        widget.insert(0, "0")
        widget.config(show=" ")  # 기본값 0을 숨기는 설정
